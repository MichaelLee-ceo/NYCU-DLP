...
args.model_dir = 'LR_rnn_size=256-predictor-posterior-rnn_layers=2-1-n_past=2-n_future=10-lr=0.0020-g_dim=128-z_dim=64-last_frame_skip=False-beta=1.0000000-tfr_start=20-cyclical=False'

# args.model_dir = 'rnn_size=256-predictor-posterior-rnn_layers=2-1-n_past=2-n_future=10-lr=0.0020-g_dim=128-z_dim=64-last_frame_skip=False-beta=0.0001000-tfr_start=20-cyclical=True'
args.log_dir = '%s/%s' % (args.log_dir, args.model_dir)
saved_model = torch.load('%s/model.pth' % args.log_dir)
print('> Loading from: {}\n> Best_epoch: {}'.format(args.model_dir, saved_model['last_epoch']))
...
with torch.no_grad():
    for i, (seq, cond) in enumerate(tqdm(test_loader)):
        ...
        pred_seq = pred(seq, cond, modules, args, device, prior=True)

...
def main():
    ...
    if args.model_dir != '':
        ...
        name = 'LR_rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f-tfr_start=%d-cyclical=%s'\
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        if epoch % 5 == 0 or epoch == (start_epoch + niter - 1):
            ...
            for i, (validate_seq, validate_cond) in enumerate(validate_loader):
                ...
                pred_seq = pred(validate_seq, validate_cond, modules, args, device, prior=True)

                if i == 5:
                    plot_pred(pred_seq[0], epoch, args)                                                   # save to png
                    plot_record(validate_seq, validate_cond, modules, epoch, args, device, prior=True)    # save to gif

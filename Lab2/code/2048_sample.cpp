...
public:
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			for (int t = 1, c = 0; c < unit; c += stat[t++]) {
				...
				if (((1 << t) & -2u) == 2048 && accu * coef > best_score) {
					best_score = accu * coef;
					best_epoch = n;
					save("weights.bin");
					std::cout << "-- saved new wieght" << std::endl;
				}
			}
			scores.clear();
			maxtile.clear();
	...
	void result() {
		std::cout << "-- Best eposide: " << best_epoch << ", Win rate: " << best_score << std::endl;
	}
